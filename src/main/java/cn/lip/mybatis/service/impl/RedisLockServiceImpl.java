 ...
 *      2.命令setnx和命令expire分开执行，setnx成功，expire失败，可能不会释放锁，出现死锁；解决方案，原子操作（当前解决方案为把过期时间设置value；也可以通过lua 脚本实现）
 ...
 *
 *           Redisson 解决方案有个解决过期时间的问题，先把当前锁的过期时间默认为设置为30s，对锁设置过期时间之后，然后生成一个后台进程，会在锁的过期时间 1/3 的时间之后，判断当前线程在执行
 *           业务代码中是否持有当前的锁,如果是，则再设置指定的过期时间以延长过期时间，以保证分布式锁的过期时间永远大于业务代码的执行时间，进而保证了多线程下业务被串行访问；
 *
 *      redis服务器方面的问题：
 *
 *      2.当前redis服务如果采用哨兵模式，线程A 成功把锁在master redis服务上面产生，并在上面设置好锁之后，同步到 slave redis 服务器之前，master宕机了，
 *          这时slave被选为master，另一个线程B再加锁的时候有成功获取锁，此时线程A还没有结束业务逻辑，就会造成锁被获取多次的问题；
 *          这个问题，在 redisson redlock 解决方案下面针对每一个机器添加一个相同的锁即可以实现；
 *
 *          【
                 主从锁失效的解决方案，在redis中不好办，解决办法用zk集群，这个方法就是利用zk的强一致性，当zk集群成功加锁之后，才响应给客户端，大致意思就是当前这个锁在zk集群中的多台机器是强一致性的，这样就不会出现主从失效的问题了；
                 但是，zk的性能没有redis性能好，如果采用zk实现分布式锁的话，性能会有一定的牺牲；
                 如果通过redis解决这种主从锁失效的方案也有，通过redlock锁实现，但是还是有一定的问题；
                 so,now，如果想要在 主从锁不失效的情况下，建议采用zk实现；否则，redis下主从就可以了，极少数的锁失效可以采用记录日志，或者后台进行人为处理操作；
 *          】
 *      3.锁和分布式锁性能的问题；
 *              如何在有锁的情况下提高分布式锁的性能？
 *
